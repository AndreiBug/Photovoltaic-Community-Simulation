...
class DataDictionaries:
    ...
    # Incarca datele o singura data si ofera acces rapid prin ID-uri.
    # Foloseste Singleton pentru eficienta.
    ...
    def __init__(self, verbose=False):
        ...
        self.appliance_df = None    # Se incarca la cerere
        self.weather_data_df = None # Se incarca la cerere
        
        # Cache pentru datele procesate (grupate per casa si statie meteo)
        self.consumption_by_house = {}  # {house_id: {epoch: kWh}}
        self.radiation_by_station = {}  # {station_id: {epoch: W/m2}}
    ...
    # ========== METODE DE ACCES PENTRU APARATE ==========
    ...
    # ========== METODE PENTRU CACHE DataFrames MARI (LAZY LOADING) ==========
    
    def get_consumption_df(self): # Returneaza DataFrame-ul cu toate datele de consum, se incarca lazy (la prima cerere) si se cache-uieste
        if self.consumption_df is None:
            ...
            if self.verbose:
                print("Consumption.csv incarcat in cache")
        return self.consumption_df
    
    def get_appliance_df(self): # Returneaza DataFrame-ul cu toate aparatele, se incarca lazy (la prima cerere) si se cache-uieste
        if self.appliance_df is None:
            if self.verbose:
                print("Incarcare Appliance.csv...")
            self.appliance_df = pd.read_csv(f"{self.db_path}/Appliance.csv")
            if self.verbose:
                print("Appliance.csv incarcat in cache")
        return self.appliance_df
    
    def get_weather_data_df(self): # Returneaza DataFrame-ul cu toate datele meteo, se incarca lazy (la prima cerere) si se cache-uieste
        if self.weather_data_df is None:
            ...
            if self.verbose:
                print("WeatherData.csv incarcat in cache")
